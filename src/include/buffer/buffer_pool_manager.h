...
namespace bustub {
     ...
     *
     * 这份代码实际上是用于实现缓存池， 具体内容你不需要理解。我们需要用到的函数有：
     ...
     * NewPageGuarded 函数用于新建一个新的 page， 为其分配它的唯一标识 PAGE ID
     * 并将该 PAGE ID 填入参数 page_id 中， 最后以 page_guard 类的形式返回它。
     * 例子：
     ...
     *
     * Fetch Page  获取页面
     * bpm_ -> FetchPageRead 和 bpm_ -> FetchPageWrite 用于通过磁盘 IO ，
     * 根据 page id 得到一个用于读的 ReadPageGuard 或者一个用于写 WritePageGuard.
     * 如果你好奇其中的细节(如缓存池)， 请私信我。
     * ReadPageGuard 会自动获取该 page 的读锁， 并在析构时释放读锁。
     * WritePageGuard 会自动获取该 page 的写锁， 并在析构时释放写锁。
     *
     *
     * Write Page  编写页面
    ...
    public:
         ...
         */
        auto NewPageGuarded(page_id_t *page_id,
                            AccessType access_type = AccessType::Unknown) -> BasicPageGuard;
        ...
        auto PgTbl() const -> std::unordered_map<page_id_t, frame_id_t> {
            return page_table_;
        }
